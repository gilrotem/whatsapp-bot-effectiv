...
function requireAdmin(req, res, next) {
  ...
  if (isProd) {
    if (!token)
      return res.status(403).json({ error: "ADMIN_TOKEN is not set" });
  ...
  Promise.resolve()
    .then(async () => {
      ...
      try {
        ...
        console.log(
          "[Telegram Webhook] Full update:",
          JSON.stringify(update, null, 2),
        );
      } catch (e) {
        console.log(
          "[Telegram Webhook] Failed to stringify update:",
          e?.message || e,
        );
      }
    ...
    .catch((err) => {
      ...
      console.error(
        "Error in telegram_webhook background handler:",
        err?.message || err,
      );
    });
});
...
async function handleStateLogic(phoneNumberId, from, message, session) {
  ...
  if (
    userInput &&
    (userInput.includes("נציג") ||
      userInput.includes("אנושי") ||
    ...
    await sendTextMessage(
      phoneNumberId,
      from,
      botConfig.messages.global_handoff_response ||
        "מעביר אותך לנציג אנושי, אנא המתן...",
    );
  ...
  switch (session.current_state) {
      ...
      if (buttonId) {
        if (buttonId === "btn_sales") {
          await updateSession(from, {
            current_state: STATES.QUALIFY_SIZE,
            intent: "sales",
          });
          ...
          await sendTextMessage(
            phoneNumberId,
            from,
            botConfig.messages.order_status_instruction,
          );
          ...
          await sendTextMessage(
            phoneNumberId,
            from,
            botConfig.messages.human_handoff_response,
          );
        }
      } else {
    ...
    case STATES.QUALIFY_SIZE:
      if (buttonId && buttonId.startsWith("size_")) {
        await updateSession(from, {
          shed_size: userInput,
          current_state: STATES.QUALIFY_FLOOR,
        });
        ...
        await sendTextMessage(
          phoneNumberId,
          from,
          botConfig.messages.validation_select_size,
        );
        await sendSizeQuestion(phoneNumberId, from);
      }
      break;
    ...
    case STATES.QUALIFY_FLOOR:
      if (buttonId && buttonId.startsWith("floor_")) {
        await updateSession(from, {
          flooring_status: buttonId,
          current_state: STATES.ASK_LOCATION,
        });
        let preText = "";
        ...
        await sendTextMessage(
          phoneNumberId,
          from,
          preText + botConfig.messages.sales_location_last_step,
        );
      } else {
        await sendTextMessage(
          phoneNumberId,
          from,
          botConfig.messages.validation_select_floor,
        );
        await sendFlooringQuestion(phoneNumberId, from);
      }
      break;
    ...
    case STATES.ASK_LOCATION:
      if (messageType === "text") {
        await updateSession(from, {
          city: userInput,
          current_state: STATES.SUMMARY_HANDOFF,
        });
        session.lead_data.city = userInput;
        await saveLead(session);

        const summaryMsg = botConfig.messages.lead_completion_summary.replace(
          "{city}",
          userInput,
        );
        ...
        await sendTextMessage(
          phoneNumberId,
          from,
          botConfig.messages.validation_enter_city,
        );
      }
      break;
...
async function sendWhatsAppMessage(phone, message, messageType = "text") {
  console.log("[WA] sending to:", phone, "type:", messageType);
